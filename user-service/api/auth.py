...
def register(db: Session, user: pym.UserCreate):
    ...
    # orm에 regdate를 찍으면 서버 시간이 제대로 안 되는 문제를 해결하기 위해 함수에 넣는 방법
...
# 엑세스 토큰 생성
# JWT : JSON Web Token
# 웹 어플리케이션에서 정보를 안전하게 전송하기 위해 사용되는 표준방법
# 주로 클라이언트/서버 간 정보 교환에 안전하게 사용할 수 있음
# HS256 : 토큰 생성 시 사용할 해시 알고리즘 중 하나
...
def authenticate(db: Session, user: pym.UserLogin):
    ...
    if not dbuser:
        return None
    if not bcrypt.checkpw(user.passwd.encode('utf-8'), dbuser.passwd):
        return None
